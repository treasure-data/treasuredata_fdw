...
TdFdwBaseQual *makeQual(AttrNumber varattno, char *opname, Expr *value,
                        bool isarray,
                        bool useOr);
...
Expr *get_em_expr(EquivalenceClass *ec, RelOptInfo *rel);
 ...
 */
Value *
colnameFromVar(Var *var, PlannerInfo *root, TdFdwPlanState * planstate)
{
         ...
         bool useOr)
{
	...
	{
	case T_Const:
		qual = palloc0(sizeof(TdFdwConstQual));
		qual->right_type = T_Const;
		qual->typeoid = ((Const *) value)->consttype;
		((TdFdwConstQual *) qual)->value = ((Const *) value)->constvalue;
		((TdFdwConstQual *) qual)->isnull = ((Const *) value)->constisnull;
		break;
	case T_Var:
		qual = palloc0(sizeof(TdFdwVarQual));
		qual->right_type = T_Var;
		((TdFdwVarQual *) qual)->rightvarattno = ((Var *) value)->varattno;
		break;
	default:
		qual = palloc0(sizeof(TdFdwParamQual));
		qual->right_type = T_Param;
		((TdFdwParamQual *) qual)->expr = value;
...
/*
 * Given a list of TdFdwDeparsedSortGroup and a TdFdwPlanState,
 * construct a list of PathKey and TdFdwDeparsedSortGroup that belongs to
 * the FDW and that the FDW say it can enforce.
 */
void computeDeparsedSortGroup(List *deparsed, TdFdwPlanState *planstate,
                              List **apply_pathkeys,
...
/*
 * Deparse a list of PathKey and return a list of TdFdwDeparsedSortGroup.
...
deparse_sortgroup(PlannerInfo *root, Oid foreigntableid, RelOptInfo *rel)
{
	...
	foreach(lc,root->query_pathkeys)
	{
		...
		TdFdwDeparsedSortGroup *md = palloc0(sizeof(TdFdwDeparsedSortGroup));
		EquivalenceClass *ec = key->pk_eclass;
		Expr *expr;
		bool found = false;

		if ((expr = get_em_expr(ec, rel)))
		{
		...
		else
		{
			...
			{
				md = (TdFdwDeparsedSortGroup *) lfirst(lc);
...
Expr *
get_em_expr(EquivalenceClass *ec, RelOptInfo *rel)
{
	ListCell   *lc_em;
...
serializeDeparsedSortGroup(List *pathkeys)
{
	...
	{
		List *item = NIL;
		TdFdwDeparsedSortGroup *key = (TdFdwDeparsedSortGroup *)
		                              lfirst(lc);
...
deserializeDeparsedSortGroup(List *items)
{
	...
	{
		ListCell *lc;
		TdFdwDeparsedSortGroup *key =
		    palloc0(sizeof(TdFdwDeparsedSortGroup));
